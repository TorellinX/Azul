 ...
 */
public class DrawboardTableCenterCandy extends JPanel {
  ...
  private BufferedImage backgroundIMGBeach;
   ...
   */
  public DrawboardTableCenterCandy(GameModel model, Controller controller, int playerNumber) {
    if (playerNumber > 2) {
   ...
   */
  public void drawCandyBackground(Graphics2D g2D){
    ClassLoader classLoader = null; //Name dieser klasse
    try {
      classLoader = Class.forName("de.lmu.ifi.sosylab.view.DrawboardTableCenterCandy").getClassLoader();
    } catch (ClassNotFoundException e) {
      throw new RuntimeException(e);
    }
    var stream = (classLoader.getResourceAsStream("back_candy.png"));
    try {
      backgroundIMGBeach = ImageIO.read(stream);
    } catch (IOException e) {
      e.printStackTrace();
    }
    g2D.drawImage(backgroundIMGBeach,0, 0,410, 900,this);
  }
