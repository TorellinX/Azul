 ...
 */
public class DrawboardTableCenterCosmic extends JPanel {
   ...
   */
  public DrawboardTableCenterCosmic(GameModel model, Controller controller, int playerNumber) {
    if (playerNumber > 2) {
  ...
  public void drawCosmicBackground(Graphics2D g2D){
    ...
    try {
      classLoader = Class.forName("de.lmu.ifi.sosylab.view.DrawboardTableCenterCosmic").getClassLoader();
    } catch (ClassNotFoundException e) {
      throw new RuntimeException(e);
    }
    var stream = (classLoader.getResourceAsStream("back_cosmic.png"));
    try {
